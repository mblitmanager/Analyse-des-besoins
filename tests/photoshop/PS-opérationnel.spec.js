...
await page.getByRole('button', { name: 'Continuer arrow_forward' }).click();
  ...
  await page.getByText("Ils ont exactement la même fonction").click();

  await expect(page.getByText("profil ICC")).toBeVisible();
  await page.getByText("Édition > Couleurs").click();

  await expect(page.getByText("sélection en objet dynamique")).toBeVisible();
  await page.getByText("Supprimer la sélection").click();
  ...
  // await page.getByText("Appliquer un dégradé linéaire ou radial directement sur l’objet masqué et ajuster la transparence globale de l’objet").click();

  // await expect(page.getByText("HDR réaliste")).toBeVisible();
  // await page.getByText("Appliquer un filtre Flou").click();

  // --- Fin ---
  // await page.getByRole("button", { name: "Terminer arrow_forward" }).click();

  await page.waitForTimeout(2500);
  await page.screenshot({ path: "photoshop/Photoshop-opérationnel-OK.png", fullPage: true });

  await page.getByRole("button", { name: "Continuer avec photoshop arrow_forward" }).click();

  await page.waitForTimeout(3000);
  await page.screenshot({ path: "photoshop/Photoshop-opérationnel.png", fullPage: true });
