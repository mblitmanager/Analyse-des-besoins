...
test('test', async ({ page }) => {
  ...
  await page.getByRole('textbox', { name: 'Ex: Assistant administratif,' }).fill('IA');
  ...
  await page.getByRole('button', { name: 'Continuer arrow_forward' }).click();
  await page.locator('div').filter({ hasText: 'quizQuel est l’objectif de' }).nth(5).click();
  ...
  await page.locator('div:nth-child(3) > .space-y-3 > .grid > label > .option-card__radio').first().click();
  await page.getByRole('button', { name: 'Continuer arrow_forward' }).click();
