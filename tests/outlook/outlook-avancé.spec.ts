...
test("Test Outlook Avancé", async ({ page }) => {
  ...
  // --- Niveau OPERATIONNEL ---
  await expect(
    page.getByText("Comment créer un dossier pour classer ses emails ?"),
  ).toBeVisible();
  await page
    .getByText("Clic droit sur Boîte de réception → Nouveau dossier")
    .click();

  await expect(
    page.getByText(
      "Quelle fonction permet d’envoyer une réponse automatique pendant ses congés ?",
    ),
  ).toBeVisible();
  ...
  await page
    .getByText(
      "Envoyer une copie en préservant la confidentialité des adresses",
    )
    .click();

  await expect(
    page.getByText("Comment demander un accusé de réception ?"),
  ).toBeVisible();
  await page.getByText("Dans les options du message").click();

  await expect(
    page.getByText("Comment effectuer une recherche d’email par expéditeur ?"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      "Quelle est la principale caractéristique d’un compte IMAP par rapport à un compte POP ?",
    ),
  ).toBeVisible();
  await page
    .getByText(
      "Les mails restent synchronisés avec le serveur et accessibles depuis plusieurs appareils",
    )
    .click();

  await expect(
    page.getByText(
      "Comment programmer l’envoi d’un email à une date spécifique ?",
    ),
  ).toBeVisible();
  await page.getByText("Options → Différer la livraison").click();

  await expect(
    page.getByText("À quoi servent les catégories de couleurs ?"),
  ).toBeVisible();
  await page.getByText("Classer et filtrer les éléments Outlook").click();

  await expect(
    page.getByText(
      "Comment transférer automatiquement un mail d’un expéditeur spécifique vers un dossier prédéfini ?",
    ),
  ).toBeVisible();
  await page.getByText("Règles → Créer une règle").click();

  await expect(
    page.getByText("Comment créer une liste de distribution ?"),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      "Quelle est l’extension d’un modèle de courrier électronique Outlook ?",
    ),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      "Quelle est l’extension du fichier utilisé pour exporter un calendrier Outlook afin qu’il puisse être importé dans un autre agenda ?",
    ),
  ).toBeVisible();
  ...
  await expect(
    page.getByText(
      "À quoi sert le mappage des champs lors de l’importation d’un fichier de contacts dans Outlook ?",
    ),
  ).toBeVisible();
  await page
    .getByText(
      "Associer les colonnes du fichier importé aux champs correspondants dans Outlook",
    )
    .click();

  await expect(
    page.getByText("Quelle fonctionnalité permet de configurer des flux RSS ?"),
  ).toBeVisible();
  await page.getByText("Paramètres des flux RSS").click();

  await expect(
    page.getByText(
      "Quelle fonctionnalité permet de créer un dossier affichant automatiquement tous les messages non lus, sans les déplacer de leur emplacement d’origine ?",
    ),
  ).toBeVisible();
