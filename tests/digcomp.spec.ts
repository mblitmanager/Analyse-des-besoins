...
test('Digitales Compétences - Full User Journey', async ({ page }) => {
  // ===== STEP 1: INTAKE FORM =====
  ...
  const formationButton = await page.getByRole('button').filter({ has: page.getByText('Digitales Compétences') }).first();
