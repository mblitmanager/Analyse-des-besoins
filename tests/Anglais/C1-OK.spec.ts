...
test("test TOEIC complet jusqu'au C1", async ({ page }) => {
  ...
  await page.locator("div:nth-child(5) > .grid > label > .option-card__radio").first().click();
  ...
  await page.locator("div:nth-child(2) > .p-6 > .grid > label:nth-child(2) > .option-card__radio").click();
  ...
  await page.locator("label").filter({ hasText: /^were$/ }).click();
