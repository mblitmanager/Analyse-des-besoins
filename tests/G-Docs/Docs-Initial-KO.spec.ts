...
test('ICDL Google Docs', async ({ page }) => {
  ...
  await page.screenshot({ path: "GoogleDocs-Initial-KO1.png", fullPage: true });
...
await page.getByRole("button", { name: "Continuer quand même" }).click();
  ...
  await page.screenshot({ path: "GoogleDocs-Initial-KO2.png", fullPage: true });

  await page.getByRole("button", { name: "Continuer" }).click();
await page.waitForTimeout(3000);
  await page.screenshot({ path: "GoogleDocs-Initial-KO3.png", fullPage: true });
