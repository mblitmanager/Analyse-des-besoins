...
test('test', async ({ page }) => {
  ...
  await page.locator('div:nth-child(4) > .flex.items-center.w-full > .formation-card__radio').click();
  ...
  await page.locator('label').filter({ hasText: 'Du dessin vectoriel' }).click();
  ...
  await page.locator('label').filter({ hasText: 'À déplacer tout un groupe d\'' }).click();
  await page.getByRole('button', { name: 'Suivant arrow_forward' }).click();
  await page.getByText('L\'outil Plume').click();
  await page.locator('label').filter({ hasText: 'À fusionner ou soustraire des' }).click();
  await page.locator('label').filter({ hasText: 'En utilisant la fonction "' }).click();
  await page.getByText('CMJN (Cyan, Magenta, Jaune,').click();
  await page.locator('label').filter({ hasText: 'Les lignes directrices (ou' }).click();
