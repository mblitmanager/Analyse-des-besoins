...
<template>
  <div class="space-y-6">
    <div class="flex items-center justify-between">
      ...
      <button @click="openModal()" class="px-4 py-2 bg-slate-900 text-white font-bold rounded-lg hover:bg-slate-800 flex items-center gap-2 shadow-sm">
    ...
    <div v-else class="grid grid-cols-1 gap-4">
      <div v-if="rules.length === 0" class="bg-white p-8 rounded-xl text-center shadow-sm border border-slate-100">
        <div class="max-w-xl mx-auto space-y-4">
          <div class="w-12 h-12 mx-auto rounded-full bg-slate-100 text-slate-500 flex items-center justify-center">
            <span class="material-icons-outlined">filter_alt</span>
          </div>
          <p class="text-slate-700 font-semibold">Aucune règle définie.</p>
          <p class="text-slate-500 text-sm">
            Commencez par créer une première règle pour piloter le filtrage des formations en mode P3.
          </p>
          <button
            type="button"
            @click="openModal()"
            class="inline-flex items-center gap-2 px-4 py-2 bg-slate-900 text-white font-bold rounded-lg hover:bg-slate-800 transition-colors"
          >
        ...
        <form @submit.prevent="saveRule" class="p-6 space-y-6">
          ...
          <div class="flex gap-3 justify-end pt-4 border-t border-gray-100">
            ...
            <button
              ...
              class="px-6 py-2 bg-slate-900 border border-slate-900 text-white font-bold rounded-xl hover:bg-slate-800 transition-all flex items-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed"
            >
