...
/**
 * Formation: digitales-comptences | Level: oprationnel | Parcours: 2
 * Niveau ordre: 3
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: Digitales Compétences Basique (TOSA)
 * - Formation 2: Digitales Compétences Opérationnel (TOSA)
 */

test("digitales-comptences-oprationnel-parcours-2 - Digitales Compétences - Opérationnel - Parcours 2", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "Digitales Compétences"
  const formationSelectors = await page.locator("*:has-text(\"Digitales Compétences\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'Digitales Compétences' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/digitales-comptences-oprationnel-parcours-2-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/digitales-comptences-oprationnel-parcours-2-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/digitales-comptences-oprationnel-parcours-2-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/digitales-comptences-oprationnel-parcours-2-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: digitales-comptences-oprationnel-parcours-2`);
});
