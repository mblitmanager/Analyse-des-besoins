...
/**
 * Formation: outlook | Level: oprationnel | Parcours: 1
 * Niveau ordre: 3
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: Outlook Basique (TOSA)
 * - Formation 2: Outlook Opérationnel (TOSA)
 */

test("outlook-oprationnel-parcours-1 - Outlook - Opérationnel - Parcours 1", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "Outlook"
  const formationSelectors = await page.locator("*:has-text(\"Outlook\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'Outlook' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/outlook-oprationnel-parcours-1-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/outlook-oprationnel-parcours-1-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/outlook-oprationnel-parcours-1-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/outlook-oprationnel-parcours-1-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: outlook-oprationnel-parcours-1`);
});
