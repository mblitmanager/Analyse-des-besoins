...
/**
 * Formation: excel | Level: p3 | Parcours: 2
 */

test("excel - p3 - Parcours 2", async ({ page, context }) => {
  // Navigate to formations
  await page.goto("/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation
  const formationSelector = page.locator("*:has-text(\"Excel\")").first();
  ...
  await page.screenshot({
    path: "./test-results/screenshots/excel-p3-parcours-2-01-positionnement-end.png",
    fullPage: true
  });
  ...
  // Screenshot results
  await page.screenshot({
    path: "./test-results/screenshots/excel-p3-parcours-2-02-results.png",
    fullPage: true
  });
  
  console.log("Done: excel p3 parcours 2");
});
