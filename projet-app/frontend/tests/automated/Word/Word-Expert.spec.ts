...
test("Word-Expert", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "Prénom" }).fill("Expert");
  ...
  //await expect(page.getByText("A quoi sert le logiciel Word ?")).toBeVisible();
  ...
  await page.getByText("document maître").click();
  ...
  await page.getByText("Volet sélection").click();
  ...
  await page.getByText("Restreindre la modification").click();
  await page.getByText("Contact GMail").click();
  ...
  await page.screenshot({ path: "Word-Expert.png", fullPage: true });
  ...
  await page.screenshot({ path: "Word-Expert2.png", fullPage: true });
});