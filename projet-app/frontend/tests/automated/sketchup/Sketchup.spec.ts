...
// Sketchup - test prérequis technique uniquement
// Attendu: ICDL Sketchup + ICDL Sketchup
test("Sketchup → ICDL Sketchup & ICDL Sketchup", async ({ page }) => {
  ...
  await page.getByRole("textbox", { name: "Ex: Assistant administratif," }).fill("AF");
  ...
  await page.locator("div:nth-child(3) > .grid > label > .option-card__radio").first().click();
  await page.locator("div:nth-child(4) > .grid > label > .option-card__radio").first().click();
  ...
  await page.screenshot({ path: "Sketchup-result.png", fullPage: true });
  // Assertion: vérifier que le parcours recommandé est ICDL Sketchup
  // const recommendation = page.locator('h3.text-xl, .text-brand-primary.font-extrabold').first();
  // await expect(recommendation).toContainText(/ICDL Sketchup/i, { timeout: 5000 });
});
