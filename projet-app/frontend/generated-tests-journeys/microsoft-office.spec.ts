...
test('Mixte Microsoft Office (Word + Excel) - Full User Journey', async ({ page }) => {
  // ===== STEP 1: INTAKE FORM =====
  ...
  const formationButton = await page.getByRole('button').filter({ has: page.getByText('Mixte Microsoft Office (Word + Excel)') }).first();
