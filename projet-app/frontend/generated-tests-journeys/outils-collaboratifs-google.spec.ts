...
test('Outils Collaboratifs Google - Full User Journey', async ({ page }) => {
  // ===== STEP 1: INTAKE FORM =====
  ...
  const formationButton = await page.getByRole('button').filter({ has: page.getByText('Outils Collaboratifs Google') }).first();
