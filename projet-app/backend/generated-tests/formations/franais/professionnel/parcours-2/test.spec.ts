...
/**
 * Formation: franais | Level: professionnel | Parcours: 2
 * Niveau ordre: 2
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: Français Technique - Voltaire
 * - Formation 2: Français Professionnel - Voltaire
 */

test("franais-professionnel-parcours-2 - Français - Professionnel - Parcours 2", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "Français"
  const formationSelectors = await page.locator("*:has-text(\"Français\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'Français' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/franais-professionnel-parcours-2-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/franais-professionnel-parcours-2-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/franais-professionnel-parcours-2-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/franais-professionnel-parcours-2-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: franais-professionnel-parcours-2`);
});
