...
/**
 * Formation: word | Level: oprationnel | Parcours: 3
 * Niveau ordre: 3
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: Word Basique (TOSA)
 * - Formation 2: Word Opérationnel (TOSA)
 */

test("word-oprationnel-parcours-3 - Word - Opérationnel - Parcours 3", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "Word"
  const formationSelectors = await page.locator("*:has-text(\"Word\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'Word' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/word-oprationnel-parcours-3-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/word-oprationnel-parcours-3-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/word-oprationnel-parcours-3-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/word-oprationnel-parcours-3-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: word-oprationnel-parcours-3`);
});
