...
/**
 * Formation: wordpress | Level: operationnel | Parcours: 2
 * Niveau ordre: 2
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: Wordpress Basique (TOSA)
 * - Formation 2: Wordpress Opérationnel (TOSA)
 */

test("wordpress-operationnel-parcours-2 - WordPress - Operationnel - Parcours 2", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "WordPress"
  const formationSelectors = await page.locator("*:has-text(\"WordPress\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'WordPress' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/wordpress-operationnel-parcours-2-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/wordpress-operationnel-parcours-2-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/wordpress-operationnel-parcours-2-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/wordpress-operationnel-parcours-2-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: wordpress-operationnel-parcours-2`);
});
