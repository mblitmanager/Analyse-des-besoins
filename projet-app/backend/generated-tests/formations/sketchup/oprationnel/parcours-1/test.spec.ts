...
/**
 * Formation: sketchup | Level: oprationnel | Parcours: 1
 * Niveau ordre: 3
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: SketchUp (ICDL)
 * - Formation 2: GIMP (ICDL)
 */

test("sketchup-oprationnel-parcours-1 - SketchUp - Opérationnel - Parcours 1", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "SketchUp"
  const formationSelectors = await page.locator("*:has-text(\"SketchUp\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'SketchUp' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/sketchup-oprationnel-parcours-1-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/sketchup-oprationnel-parcours-1-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/sketchup-oprationnel-parcours-1-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/sketchup-oprationnel-parcours-1-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: sketchup-oprationnel-parcours-1`);
});
