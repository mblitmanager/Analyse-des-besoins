...
/**
 * Formation: powerpoint | Level: oprationnel | Parcours: 3
 * Niveau ordre: 3
 * Seuil de réussite: 4%
 * 
 * Formations proposées:
 * - Formation 1: PowerPoint Basique (TOSA)
 * - Formation 2: PowerPoint Opérationnel (TOSA)
 */

test("powerpoint-oprationnel-parcours-3 - PowerPoint - Opérationnel - Parcours 3", async ({ page, context }) => {
  // Navigate to formations page
  await page.goto("https://ns-conseil-ab.mbl-service.com/formations");
  await page.waitForLoadState("networkidle");
  
  // Select formation - search for "PowerPoint"
  const formationSelectors = await page.locator("*:has-text(\"PowerPoint\")").all();
  ...
  if (!formationFound) {
    console.log("Warning: Formation 'PowerPoint' not found on the page");
  }
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/powerpoint-oprationnel-parcours-3-01-positionnement-end.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 1 saved: ./test-results/screenshots/powerpoint-oprationnel-parcours-3-01-positionnement-end.png`);
  } catch (e) {
  ...
  try {
    await page.screenshot({
      path: "./test-results/screenshots/powerpoint-oprationnel-parcours-3-02-results.png",
      fullPage: true
    });
    console.log(`📸 Screenshot 2 saved: ./test-results/screenshots/powerpoint-oprationnel-parcours-3-02-results.png`);
  } catch (e) {
  ...
  console.log(`✅ Test completed: powerpoint-oprationnel-parcours-3`);
});
